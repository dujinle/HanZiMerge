...
let WxBannerAd = {
    ...
    createBannerAd: function () {
        if (typeof wx !== 'undefined') {
            ...
            if (sysInfo && sysInfo.SDKVersion && sysInfo.SDKVersion !== '' && sysInfo.SDKVersion.slice(0, 5).replace(/\./g, "") >= 204) {
                ...
                if (this.isCreateSuccess && !GData.alyunGameCF.refreshBanner) {
                ...
                try {
                    this.bannerAd = wx.createBannerAd({
                        adUnitId: 'adunit-16b06562ac778585',
                        style: {
                            left: 0,
                            top: 0,
                            width: 300
                    ...
                    if (this.bannerAd) {
						...
						this.bannerAd.onResize(res => {
							...
							if (self.bannerAd && self.bannerAd.style) {
								// 适配iphoneX
								var fitOffsetY = GData.phoneModel == 'Normal' ? 0:0.01;
								console.log('fitOffsetY:', fitOffsetY);
								var buttomHeight = fitOffsetY;
								if (GData.phoneModel != 'Normal') {
    ...
    destroyBannerAd: function () {
        if (this.bannerAd) {
            if (!GData.alyunGameCF.refreshBanner) {
                this.hideBannerAd();
                return;
            }
            try {
    ...
    forceDestroyBannerAd: function () {
        if (this.bannerAd && GData.alyunGameCF.refreshBanner) {
            try {
                this.bannerAd.destroy();
                this.bannerAd = null;
            } catch (error) {
